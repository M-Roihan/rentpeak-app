    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-16">
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="font-bold text-blue-600 text-xl tracking-tight" style={{ fontFamily: '"Sora", sans-serif' }}>
            RENTPEAK
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-2xl mx-auto px-4 mt-8">
        <h1 className="text-3xl font-bold text-slate-900 mb-8" style={{ fontFamily: '"Sora", sans-serif' }}>Profil Saya</h1>

        {loading ? (
          /* Loading Skeleton */
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8 animate-pulse flex items-center gap-6">
              <div className="w-24 h-24 bg-slate-200 rounded-full flex-shrink-0"></div>
              <div className="space-y-3 flex-1">
                <div className="h-6 bg-slate-200 rounded w-1/2"></div>
                <div className="h-4 bg-slate-200 rounded w-1/3"></div>
                <div className="h-6 bg-slate-200 rounded w-24 rounded-full mt-2"></div>
              </div>
            </div>
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8 animate-pulse space-y-6">
          ...
          userData && (
            <div className="space-y-6">
              {/* SECTION 1 - Info Akun */}
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8 flex flex-col sm:flex-row items-center sm:items-start gap-6 relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-24 bg-blue-50/50"></div>
                
                <div className="w-24 h-24 bg-blue-600 text-white rounded-full flex items-center justify-center text-3xl font-bold border-4 border-white shadow-lg relative z-10 flex-shrink-0" style={{ fontFamily: '"Sora", sans-serif' }}>
                  {getInitials(userData.nama)}
                </div>
                
                <div className="text-center sm:text-left relative z-10 w-full pt-2">
                  <h2 className="text-2xl font-bold text-slate-900 mb-1">{userData.nama}</h2>
                  <p className="text-slate-500 mb-3">{userData.email}</p>
                  
                  <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-4 mt-4 pt-4 border-t border-slate-100">
                    <div>
                    ...
                    <div>
                      <span className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Bergabung Sejak</span>
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
                <h3 className="text-xl font-bold text-slate-800 mb-6 border-b border-slate-100 pb-4">Lengkapi Data Diri</h3>
                ...
                <form onSubmit={handleSimpan} className="space-y-5">
                  <div>
                    ...
                    <input 
                      type="text" 
                      maxLength={16}
                      className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all font-mono text-slate-700"
                      placeholder="Masukkan 16 digit NIK"
                      value={formNIK}
                      onChange={(e) => setFormNIK(e.target.value.replace(/[^0-9]/g, ''))} // Hanya angka
                    />
                    <p className="text-xs text-slate-500 mt-2 font-medium">16 digit angka sesuai KTP Anda.</p>
                  </div>
