  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-16">
      ...
      <div className="max-w-3xl mx-auto px-4 mt-8">
        <h1 className="text-3xl font-bold text-slate-900 mb-8" style={{ fontFamily: '"Sora", sans-serif' }}>Riwayat Peminjaman</h1>
        ...
        {loading ? (
          ...
          /* State Kosong */
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-12 text-center flex flex-col items-center mt-4">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-20 w-20 text-slate-300 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <h3 className="text-xl font-bold text-slate-700 mb-2">Belum ada riwayat peminjaman</h3>
            <p className="text-slate-500 mb-6">Kamu belum pernah menyewa barang apapun.</p>
            <Link href="/" className="bg-blue-600 text-white font-bold px-6 py-3 rounded-xl hover:bg-blue-700 transition-colors">
              Mulai Menyewa
            </Link>
          </div>
        ) : (
          ...
          <div className="space-y-6">
            {bookings.map((booking) => (
              <div key={booking.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden hover:border-blue-200 transition-all">
                
                {/* Header Card (Kode + Status) */}
                <div className="bg-slate-50 p-4 sm:px-6 flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-100 gap-3 sm:gap-0">
                  <div className="font-mono text-sm font-medium text-slate-500 flex items-center">
                ...
                <div className="p-4 sm:p-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="md:col-span-2">
                      <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Item yang Disewa</h4>
                      <div className="space-y-3">
                        {booking.details.map((detail, idx) => (
                          <div key={idx} className="flex justify-between items-start">
                            <div>
                              <p className="font-bold text-slate-800">{detail.barang.nama}</p>
                            ...
                            <div className="text-slate-500 font-medium text-sm bg-slate-50 px-2.5 py-1 rounded-md border border-slate-100">
                              x{detail.jumlah}
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="mt-5 pt-4 border-t border-slate-100 grid grid-cols-2 gap-4">
                        <div>
                          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Durasi Sewa</p>
                        ...
                        <div>
                          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Metode Bayar</p>
